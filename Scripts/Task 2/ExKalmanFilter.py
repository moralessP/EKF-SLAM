...
class ExKalmanFilter:
  ...
  def animate(self,i): 
    ...
    #### MEAS FROM SENSOR
    if abs(self.psi1[i]) <=np.pi/4 and self.r1[i] != 0:
    ...
    if abs(self.psi2[i]) <=np.pi/4 and self.r2[i] != 0:
      self.ax.plot(self.x[i] + self.r2[i] * np.cos(self.theta[i] + self.psi2[i]), self.y[i] + self.r2[i] * np.sin(self.theta[i] + self.psi2[i]),"x", label ="Meaurements", color="green")
    ####
...
def main():
  ...
  data = np.loadtxt("Data/data2.txt", delimiter=None, dtype=float)
